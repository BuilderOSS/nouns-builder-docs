...
This schema defines a how users of Nouns Builder can to submit *Propdates* to DAO proposals, i.e. *updates*, *comments*, and *milestone-related actions* to proposals using the [Ethereum Attestation Service (EAS)](https://docs.attest.sh).
...
The schema with the UID `0x8bd0d42901ce3cd9898dbea6ae2fbf1e796ef0923e7cbb0a1cecac2e42d47cb3` enables Nouns Builder Propdates. It is is deployed on-chain on all networks supported by Nouns Builder, consisting of mandatory and optional fields. The payload dispalyed on the frontend is wrapped in `message`, which MAY contain a *JSON* string.
