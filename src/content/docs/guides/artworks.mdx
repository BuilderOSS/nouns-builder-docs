...
Refer to the [Configuring the NFT's Images guide](/guides/img-config) to learn how to setup your NFT artwork.
...
3. Upload your artwork folder:
   - Structure: Organise into subfolders (e.g., `0-background/`, `1-body/`) containing variant files.
   - All files must be of the same type, e.g., all PNG, all SVG, all GIF (**coming to production soon**).
...
- All NFTs will reflect the updates, even those already minted if a certain file is replaced.
- To "remove" a variant, replace it with a transparent file, but this may cause visual on already minted NFTs issues if selected.
- All files must match the existing file type (e.g., if current is PNG, add PNG only).

## Add Artwork Proposal

This proposal type adds new variants to existing layers or introduces entirely new layers. It is an additive process and does not overwrite the full set.
...
- If adding to multiple layers, upload them in one folder—duplicates will replace, but this is additive overall.
- Untested for very large additions (e.g., >500 variants per layer); transaction gas limits may apply.
...
- **File Count and Size**: 
  ...
  - File limit: Up to 150 files can be uploaded at once to the v.3 Pinata API, which is why we have rolled back to the legacy API until further notice.
...
- **GIF**: Animated; **PROTOTYPICAL IMPLEMENTATION, proceed with caution on staging**, but all files in the collection must be GIF (convert static layers to single-frame GIFs if needed).
- **PDF**: Not supported (out of scope).
