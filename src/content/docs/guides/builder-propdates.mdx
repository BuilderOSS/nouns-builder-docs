...
title: How to Post Propdates
---

## Overview

In addition to creating milestone-based escrow proposals and requesting the release of milestones, users and DAO members specifically can now **post updates**, so called *"Propdates"* directly within the proposal feed. These updates use the [Ethereum Attestation Service (EAS)](https://docs.attest.org/) to record comments and updates on-chain in a secure and verifiable way.

Users can access the Propdate functionality by navigating to `Activity` of their DAO, select a specific proposal, and scrolling down.
...
Each update or release request is stored as an **on-chain attestation** using EAS, read more at the [Attestations Reference](./src/content/docs/reference/attestations.mdx). Propdates you post will appear in the **"Propdates"** feed under the proposal.
