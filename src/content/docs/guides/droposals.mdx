...
Enable **Use revenue split** under **Revenue** to share mint proceeds and royalties between multiple recipients.

Add each recipient and its allocation. Recipient addresses must be unique and the allocations must total 100% before you can create the split.

<img
  src="/guides/builderGuide/droposal-split-recipients.png"
  alt="Revenue split recipients and their allocations"
  width="500"
/>

Creating the split requires a separate wallet transaction. Once deployed, its address becomes the Droposal's payout address; the Droposal itself still needs to pass through governance.
