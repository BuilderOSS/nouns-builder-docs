...
Creating the split requires a separate wallet transaction. Once deployed, its address becomes the Droposal's payout address; the Droposal itself still needs to pass through governance. The split contract forwards proceeds and royalties to its configured recipients according to their allocations.
...
Add the Builder UI withdrawal flow here once the withdrawal control is exposed on the Droposal. The contract-level `withdraw` function and recipient rules are documented in [Withdrawing Funds](#withdrawing-funds) below.
