...
1. Open your DAO and select the **`Proposals`** tab.
2. Click **`Create Proposal`** and choose **`Escrow Milestones`** from the proposal type menu.

![Proposal transaction types](/guides/escrowGuide/escrow_milestones.png)
...
When Nouns Builder recognises a Smart Invoice escrow deployment, the proposal review displays a readable transaction summary. Check the milestone amounts and total, client, provider, resolver, payout address, token and safety valve date before submitting. Select **Raw** to inspect the underlying transaction data.

![Decoded Smart Invoice escrow transaction summary](/guides/escrowGuide/escrow-transaction-summary.png)
