...
Nouns Builder requires you to authenticate with **Sign-In with Ethereum (SIWE)** after connecting your wallet. The signature proves that you control the address without creating a transaction or costing gas.

**Steps to set up:**

1. Download and install your preferred wallet.
2. Create a new wallet and back up your seed phrase securely.
3. Connect your wallet to [nouns.build](https://nouns.build/).
4. Sign the SIWE message when prompted. This authenticates your session without creating a transaction or costing gas.
5. Browse and join a DAO by participating in an auction or acquiring a governance token.

![Wallet connection options on Nouns Builder](/onboarding/sign-in/connect-wallet.png)

If you use a Safe, follow the separate instructions for [signing in with a Safe](/onboarding/sign-in-with-ethereum/#signing-in-with-a-safe). An owner wallet connects and signs the authentication message on the Safe's behalf.
...
Before launching your DAO on mainnet, it's a good idea to test your setup on a supported testnet, such as Base Sepolia.
