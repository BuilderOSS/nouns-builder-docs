...
For DAOs using the original Governor contracts, or an upgraded Governor with its update period disabled, a submitted proposal moves through the following stages:
...
:::note[DAOs with updateable proposals enabled]

These proposals begin in an **Updatable** stage before moving to **Pending**. The optional update period is an additional stage provided by the upgraded Governor; it does not replace or alter the legacy lifecycle for DAOs that have not enabled it. See [Updating a Proposal](/guides/updating-proposals/).
...
Open your DAO, select the **Proposals** tab and click **Create proposal**.
...
| [**Stream Tokens**](/guides/sablier-proposal/) | Set up continuous token payments over time (powered by Sablier) |
| [**Airdrop Tokens**](/guides/sablier-proposal/) | Distribute tokens via Sablier merkle campaigns |
| [**Milestone Payments**](/guides/builder-escrow-proposal/) | Schedule token releases tied to milestones |
| [**Mint Governance Tokens**](/guides/mint_gov_tokens/) | Mint governance tokens to specific addresses |
| **WalletConnect** | Connect to a dApp and execute transactions via WalletConnect |
| [**Nominate Delegate**](/guides/builder-escrow-proposal/) | Nominate a delegate for milestone payments or token streams |
...
| [**Coining**](/guides/coining/) | Create a proposal to mint a Content or DAO Coin |
| [**Droposal: Single Edition**](/guides/droposals/) | Create a single-edition ERC-721 collection droposal |
| [**Pause Auctions**](/guides/auctions/) | Pause DAO auctions |
| [**Add Artwork**](/guides/artworks/) | Add new artwork layers to your DAO's NFT collection |
| [**Replace Artwork**](/guides/artworks/) | Replace an existing artwork layer in your collection |
...
Once the transaction confirms, a proposal without an enabled update period appears with a **Pending** status and moves to **Active** once the Voting Delay has elapsed. In a DAO with updateable proposals enabled, it appears as **Updatable** first and moves to **Pending** when the fixed update period ends.
...
## Related Proposal Workflows

- Use a [Proposal Candidate](/guides/proposal-candidates/) to develop an idea, collect member signals and gather sponsor signatures before formal submission.
- If the DAO has upgraded its Governor and enabled an update period, see [Updating a Proposal](/guides/updating-proposals/) to revise a submitted proposal before voting begins.
- For the contract-level distinction between original, upgraded-but-disabled and update-enabled Governors, see the [Governor proposal lifecycle](https://github.com/BuilderOSS/nouns-protocol/blob/e6679a20b38e2a9c35b7dbd75adf5043999dce75/docs/governor-proposal-lifecycle.md) and [upgrade runbook](https://github.com/BuilderOSS/nouns-protocol/blob/e6679a20b38e2a9c35b7dbd75adf5043999dce75/docs/upgrade-runbook.md).
